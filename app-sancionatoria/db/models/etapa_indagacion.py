...
class EtapaIndagacion(Base):
    __tablename__ = 'etapa_indagacion'
    ...
    expediente = relationship("Expediente", back_populates="etapa_indagacion")
    acto_administrativo = relationship("ActoAdministrativo", foreign_keys=[acto_administrativo_id])
    anexos = relationship("DocumentoAnexo", primaryjoin="and_(DocumentoAnexo.etapa_tipo=='indagacion', foreign(DocumentoAnexo.etapa_ref_id)==EtapaIndagacion.id)", viewonly=True)
