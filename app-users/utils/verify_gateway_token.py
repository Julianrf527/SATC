...
def verify_service_token(request: Request) -> bool:
    """
    Verifica que la petición venga de otro microservicio autenticado.
    Solo valida el X-Gateway-Token, no requiere user_id.
    Usado para llamadas servicio-a-servicio.
    """
    gw_token = request.headers.get("x-gateway-token")
    if not gw_token or gw_token != SECRET_GATEWAY:
        raise HTTPException(status_code=403, detail="Gateway token inválido")
...
def verify_permission(request: Request, required_permission: str) -> int:
    ...
    Args:
        ...
        required_permission: Nombre del permiso requerido (ej: "admin_gestionar usuarios")
