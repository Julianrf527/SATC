...
from sqlalchemy import insert, select, update, and_, func, desc, union_all, literal
...
from db.models.etapa_concepto import EtapaConcepto
...
from db.models.recurso_afectado import RecursoAfectado
from db.models.causa import Causa
...
class NuevoExpedienteSchema(BaseModel):
    radicado : str
    fecha_radicado : date
    vereda_id: int
...
async def obtener_causas(
    ...
    try:
        ...
        logger.error(f"Error en obtener_causas: {e}")
        raise HTTPException(status_code=500, detail="Error interno del servidor")
#Revisado
...
async def obtener_quejosos(
    ...
    try:
        ...
        logger.error(f"Error en obtener_quejosos: {e}")
        raise HTTPException(status_code=500, detail="Error interno del servidor")
#Revisado
...
async def crear_quejoso(
    ...
    try:
        ...
        new_quejoso = Quejoso(
            ...
            telefono=quejoso.telefono,
            correo=quejoso.correo
        )
        db.add(new_quejoso)
        await db.commit()
        await db.refresh(new_quejoso)
        return JSONResponse(content={"ok": True, "data": new_quejoso}, status_code=201)
    except Exception as e:
        logger.error(f"Error en crear_quejoso: {e}")
        raise HTTPException(status_code=500, detail="Error interno del servidor")
...
async def obtener_expediente_completo_por_expediente_id(
    ...
    try:
        ...
        concepto_q = select(
            EtapaConcepto.expediente_id,
            EtapaConcepto.fecha_creacion,
        ...
        if expediente[0]:
            stmr_rec = (
                ...
                .where(ExpedienteRecurso.expediente_id == expediente[0])
            )
            res_rec = await db.execute(stmr_rec)
            recursos = res_rec.scalars().all()
            recursos = [r.id for r in recursos]

        #Involucrados - TODA LA DATA (como en /file/{encargado_id})
        involucrados_map = await get_involved_by_expedientes_ids(db, [expediente[0]], GATEWAY_URL)

        #Veredas (map id -> {id, name})
        vereda= {}
        if expediente[4]:
            res_ver = await db.execute(
                select(Vereda.id, Vereda.nombre)
                .where(Vereda.id == expediente[4])
            )
            res_ver = res_ver.fetchone()
            vereda = {id: res_ver[0], "nombre": res_ver[1]} if res_ver else {}

        #Armar respuesta con toda la data
        exp_dict = {
            "id": expediente[0],
            "radicado": expediente[1],
            "fecha_radicado:": expediente[2].isoformat() if expediente[2] else None,
            "recurso_afectado": recursos,
            "direccion": expediente[3],
            "vereda": vereda,
            "ultima_etapa": etapa_actual.c.tipo if etapa_actual.c.tipo else None,
            "involucrados": involucrados_map,
        }
        ...
        logger.error(f"Error en obtener_expediente_completo_por_expediente_id: {e}", exc_info=True)
...
async def actualizar_informacion_expediente(
    ...
    try:
        ...
        recursos_anteriores = [
            ...
            for row in res_recursos_ant.fetchall()
        ]

        # Obtener quejosos anteriores
        stmt_quejosos_ant = (
            select(Quejoso.id, Quejoso.nombre, Quejoso.numero_documento)
        ...
        quejosos_anteriores = [
            {"id": row[0], "nombre": row[1], "documento": row[2]}
        ...
        radicados_anteriores = [
            {"id": row[0], "radicado": row[1]}
            for row in res_radicados_ant.fetchall()
        ]

        # Obtener nombre de causa anterior
        causa_anterior_nombre = None
        if expediente_actual.causa_id:
            res_causa_ant = await db.execute(
                select(Causa.nombre).where(Causa.id == expediente_actual.causa_id)
            )
            causa_anterior_nombre = res_causa_ant.scalar_one_or_none()
        ...
        # DATOS ANTERIORES COMPLETOS (sin eliminar nada)
        datos_anteriores = {
            "id": expediente_actual.id,
            ...
            "vereda_nombre": vereda_anterior_nombre,
            "causa_id": expediente_actual.causa_id,
            "causa_nombre": causa_anterior_nombre,
            "fecha_creacion": expediente_actual.fecha_creacion.isoformat() if expediente_actual.fecha_creacion else None,
            "encargado_id": expediente_actual.encargado_id,
        ...
        quejosos_nuevos = []
        if expediente.quejosos:
            for quejoso_id in expediente.quejosos:
                insert_stmt = insert(QuejosoExpediente).values(
        ...
        radicados_nuevos = []
        if expediente.radicados_asociados:
            for radicado in expediente.radicados_asociados:
                insert_stmt = insert(RadicadoAsociado).values(
                    expediente_id=expediente_actual.id,
                    radicado=radicado
                ).on_conflict_do_nothing()
                await db.execute(insert_stmt)
            ...
            radicados_nuevos = [
                {"id": row[0], "radicado": row[1]}
                for row in res_radicados_new.fetchall()
            ]
        
        # ACTUALIZAR RELACIONES (Recursos)
        recursos_nuevos = []
        if expediente.recurso:
            ...
            recursos_nuevos = [
                {"id": row[0], "nombre": row[1]}
                for row in res_recursos_new.fetchall()
            ]
        
        # OBTENER NOMBRES NUEVOS PARA AUDITORÍA
        # Nombre de causa nueva
        res_causa_new = await db.execute(
            select(Causa.nombre).where(Causa.id == expediente.causa)
        )
        causa_nueva_nombre = res_causa_new.scalar_one_or_none()

        # Nombre de vereda nueva
        res_vereda_new = await db.execute(
            select(Vereda.nombre).where(Vereda.id == expediente.vereda)
        )
        vereda_nueva_nombre = res_vereda_new.scalar_one_or_none()

        # DATOS NUEVOS COMPLETOS (sin eliminar nada)
        datos_nuevos = {
            "id": expediente_actual.id,
            "radicado": expediente.radicado,
            ...
            "descripcion": expediente.descripcion,
            "vereda_id": expediente.vereda,
            "vereda_nombre": vereda_nueva_nombre,
            "causa_id": expediente.causa,
            "causa_nombre": causa_nueva_nombre,
            "fecha_creacion": expediente_actual.fecha_creacion.isoformat() if expediente_actual.fecha_creacion else None,
            "encargado_id": expediente_actual.encargado_id,
        ...
        audit_result = await insert_log(
            ...
            descripcion=f"Actualización completa de expediente '{expediente_actual.radicado}' → '{expediente.radicado}'",
            expediente_radicado=expediente.radicado,
            id_registro=str(expediente_actual.id),
        ...
        await db.rollback()
        logger.error(f"Error al actualizar expediente {expediente_id}: {str(e)}")
...
async def obtener_expedientes_para_vista(
    ...
    try:
        ...
        # Obtener involucrados
        involucrados_map = await get_involved_by_expedientes_ids(db, expediente_ids, GATEWAY_URL)
...
async def obtener_expedientes_por_encargado(
    ...
    try:
        ...
        stmt = (
            ...
            .outerjoin(etapa_actual, etapa_actual.c.expediente_id == Expediente.id)
            .where(Expediente.encargado_id == encargado_id)
        )

        res = await db.execute(stmt)
        expedientes_raw = res.mappings().all()  # mappings() en lugar de índices posicionales
...
async def agregar_expediente(
    ...
    try:
        ...
        # Validar duplicados en una sola query
        duplicado = await db.execute(
            select(Expediente.radicado, Expediente.expediente).where(
                (Expediente.radicado == expediente.radicado) |
                (Expediente.expediente == expediente.expediente)
            )
        )
        duplicado = duplicado.mappings().first()

        if duplicado:
            if duplicado["radicado"] == expediente.radicado:
        ...
        # Crear expediente
        nuevo_expediente = NuevoExpedienteSchema(
            radicado=expediente.radicado,
            fecha_radicado=expediente.fecha_radicado,
            vereda_id=expediente.vereda,
        ...
        # Obtener datos relacionados para auditoría en una query
        quejosos = (await db.execute(
            select(Quejoso.id, Quejoso.nombre, Quejoso.numero_documento)
        ...
        # Auditoría
        user_info = get_user_info([user_id]).get(user_id, {"documento": "N/A", "nombre": "N/A"})
        ...
        audit_result = await insert_log(
            ...
            expediente_id=expediente_id,
            expediente_radicado=expediente.radicado,
            id_registro=expediente.radicado,
            datos_nuevos={
                "radicado": expediente.radicado,
                "fecha_radicado": expediente.fecha_radicado.isoformat() if expediente.fecha_radicado else None,
                ...
                "recursos": recursos,
                "quejosos": quejosos,
                "radicados_asociados": expediente.radicados_asociados,
            }
        )
        ...
        raise HTTPException(status_code=500, detail="Error al agregar expediente.")
...
async def actualizar_encargado_de_expediente(
    ...
    try:
        ...
        user_info = user_info.get(user_id, {"documento": "N/A", "nombre": "N/A"})
        ...
        if encargado_id != 0:
            notif_result = await create_user_notification(
                ...
                ruta=f"/infracction/{expediente_id}",
        ...
        if new_value:
            for expediente_actualizado in updated:
                await create_user_notification(
                    ...
                    ruta=f"/infracction/{expediente_actualizado['id']}",
