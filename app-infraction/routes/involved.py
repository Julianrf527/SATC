...
async def vincular_involucrado_a_expediente(
    ...
    try:
        ...
        stmt_expediente = select(Expediente.id,Expediente.radicado, Expediente.fecha_radicado).where(
        ...
        datos_expediente = {
            ...
            "fecha_radicado": getattr(expediente, "fecha_radicado", None)
        }
...
async def desvincular_involucrado_a_expediente(
    ...
    try:
        user_id = verify_gateway_token(request)["user_id"]
        stmr = select(Expediente.id, Expediente.radicado, Expediente.fecha_radicado).where(
            Expediente.id == expediente_id,
            Expediente.encargado_id == int(user_id)
        )
        ...
        stmt_notif = delete(Notificacion).where(
            and_(
                Notificacion.expediente_id == expediente_id,
                Notificacion.involucrado_id == select(ExpedienteInvolucrado.involucrado_id).where(
        ...
        if old_link:
            ...
            datos_expediente = {
                ...
                "fecha_radicado": getattr(exp_obj, "fecha_radicado", None) if exp_obj else None
            }
            datos_anteriores = {
                "involucrado": datos_involucrado,
                "expediente": datos_expediente
...
async def obtener_involucrados_por_expediente(
    ...
    try:
        ...
        stmt = select(ExpedienteInvolucrado.involucrado_id).where(ExpedienteInvolucrado.expediente_id == expediente_id)
        ...
        involved_data = get_involucrados_by_ids(db, [rel.involucrado_id for rel in result])  # Ajustar según la implementación real
