...
# Secreto propio de cada microservicio que llama a app-involved: la identidad
# del caller la determina qué secreto validó la firma, no el campo `service`
# del payload.
...
def verify_gateway_token(request: Request) -> dict:
    """Valida que la petición llegue del API Gateway y devuelve la identidad del usuario."""
    ...
    if not user_id or not rol_id:
        raise HTTPException(status_code=403, detail="Identidad no enviada por el gateway")

    # Una cabecera no numérica (gateway mal configurado o header falsificado)
    # es un 403, no el 500 que produciría el ValueError de int().
    try:
        return {"user_id": int(user_id), "rol_id": int(rol_id)}
    except ValueError:
        raise HTTPException(status_code=403, detail="Identidad inválida enviada por el gateway")
