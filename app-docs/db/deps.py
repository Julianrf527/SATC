...
async def get_db_managed(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    # RequestValidationError se deja pasar: FastAPI la lanza dentro de esta dependency al fallar la
    # validación de un parámetro, y ya produce el 422 correcto por sí sola; atraparla la convertía en 500.
    try:
        yield db
