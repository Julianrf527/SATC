...
const TIPO_INFORME = "SEGUIMIENTO";
...
export default function Seguimiento({
  ...
  const [informeTecnico, setInformeTecnico] = useState<InformeTecnico | null>(null);
  ...
  const handleCreateStage = async () => {
    try {
      ...
      const res = await apiCall(
        API_CONFIG.ENDPOINTS.INFRACTION_CREATE_REPORT(expedienteId, TIPO_INFORME),
        { method: "POST" },
      );
      if (res.ok) {
        ...
        setToast({ id: Date.now(), message: res.error || "Error al crear la etapa", type: "error" });
      }
    } catch {
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiCall(
        API_CONFIG.ENDPOINTS.INFRACTION_GET_REPORT(expedienteId, TIPO_INFORME),
        { method: "GET" },
      );
      ...
      if (res.ok) {
        ...
        setIsCreable(res.creable ?? false);
        setCreableMsg(res.creable_msg ?? null);
        setToast({ id: Date.now(), message: res.error || "Error al cargar el seguimiento", type: "error" });
      }
    } catch {
      setToast({ id: Date.now(), message: "Error al cargar el seguimiento", type: "error" });
    } finally {
      ...
      <div className="card-body">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-xs font-medium text-base-content/60 uppercase tracking-wide">Fecha Programación</p>
              <p className="text-sm font-semibold">{formatDate(informeTecnico?.fecha_programacion_visita)}</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-xs font-medium text-base-content/60 uppercase tracking-wide">Fecha Recibido</p>
              <p className="text-sm font-semibold">{formatDate(informeTecnico?.fecha_recibido_informe)}</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-xs font-medium text-base-content/60 uppercase tracking-wide">Fecha Aceptación</p>
