...
import type { TipoAfectacion } from "../../../types/infraccionApp";
...
export default function AdvancedFiltersModal({
  ...
  const handleToggleTipo = (id: number) =>
    setFilters((prev) => {
      const cur = prev.tipo_afectacion_ids || [];
      return { ...prev, tipo_afectacion_ids: cur.includes(id) ? cur.filter((v) => v !== id) : [...cur, id] };
    });
  ...
  const handleApply = () => {
    ...
    if (filters.tipo_afectacion_ids?.length) clean.tipo_afectacion_ids = filters.tipo_afectacion_ids;
  ...
  const recursosFiltro = filters.recurso_ids?.length
    ? recursoAfectadoList.filter((r) => filters.recurso_ids!.includes(r.id))
    : recursoAfectadoList;
      ...
      <div className="bg-base-100 rounded-2xl w-full max-w-2xl mx-4 shadow-2xl border border-base-300 max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div>
            <p className="text-xs font-semibold text-base-content/50 uppercase tracking-wider mb-3">
              Tipos de afectación
              {(filters.tipo_afectacion_ids?.length ?? 0) > 0 && (
                <span className="badge badge-success badge-xs ml-2">{filters.tipo_afectacion_ids!.length}</span>
              )}
            </p>
            <div className="border border-base-300 rounded-lg max-h-40 overflow-y-auto">
              {recursosFiltro.map((recurso) => {
                const tipos = tipoAfectacionList.filter((t) => t.recurso_id === recurso.id);
                if (tipos.length === 0) return null;
                return (
                  <div key={recurso.id}>
                    <p className="px-3 pt-2 pb-1 text-xs font-semibold text-base-content/40 uppercase tracking-wide">
                      {recurso.nombre}
                    </p>
                    {tipos.map((t) => (
                      <label key={t.id} className="flex items-center gap-2 cursor-pointer hover:bg-base-200 px-3 py-1.5">
                        <input
                          type="checkbox"
                          className="checkbox checkbox-success checkbox-xs"
                          checked={filters.tipo_afectacion_ids?.includes(t.id) || false}
