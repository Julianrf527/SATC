...
export default function FileManageLayout({ setToast }: Props) {
  const { user } = useAuth();
  ...
  const [causaList, setCausaList] = useState<ModeloGenerico[]>([]);
  ...
  useEffect(() => {
    if (location.state?.radicadoToSelect && expedienteList.length > 0) {
      const fileToSelect = expedienteList.find(
        (f) => f.radicado === location.state.radicadoToSelect,
      );

      if (fileToSelect) {
        setExpedienteSeleccionado(fileToSelect);
      } else {
  ...
  useEffect(() => {
    const getResources = async () => {
      try {
        setIsLoading(true);
        const [municipios, recursos, causas, quejosos] = await Promise.all([
          ...
          apiCall(API_CONFIG.ENDPOINTS.INFRACTION_CAUSES, { method: "GET" }),
        ...
        if (causas.ok) {
          setCausaList(causas.data);
        } else {
          setToast({
            id: Date.now(),
            message: causas.detail || "Error al cargar las causas",
            type: "error",
          });
        }
      ...
      try {
        if (user) {
          const res = await apiCall(
            API_CONFIG.ENDPOINTS.INFRACTION_BY_USER(user.user_id),
          );
          if (res.ok) {
  ...
  return (
    <>
      <div className="flex h-[calc(100vh-4rem)] bg-gray-100">
        {/* SIDEBAR IZQUIERDO - expedienetList */}
        <div
          className={`${
            showExpedienteList ? "w-80" : "w-0"
        ...
        {/* CONTENIDO PRINCIPAL - DetalleInfraccion */}
