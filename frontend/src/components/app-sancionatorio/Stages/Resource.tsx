...
export default function Resource({
  ...
  const fetchRecurso = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiCall(API_CONFIG.ENDPOINTS.FILE_RESOURCE(expedienteId), {
        method: "GET",
      });
  ...
  const tipoEtapaId =
    recursoData?.tipo_etapa_id || localRecurso?.tipo_etapa_id || null;
  const creable = recursoData?.creable ||
    localRecurso?.creable || { status: true, msg: "" };
  ...
  // Handler para cuando se actualiza el acto admin de decisión
  const handleActoAdminDecisionUpdate = async (actoAdmin: ActoAdminData) => {
