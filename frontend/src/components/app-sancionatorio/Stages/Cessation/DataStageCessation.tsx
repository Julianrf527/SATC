...
export default function DataStageCessation({
  data,
  tipoMedida,
  setToast,
  etapaId,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (localData?.id) {
        res = await apiCall(
          API_CONFIG.ENDPOINTS.FILE_CESSATION_UPDATE(localData.id),
          {
            method: "PUT",
            headers: {
        ...
        res = await apiCall(API_CONFIG.ENDPOINTS.FILE_CESSATION_CREATE, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
      ...
      if (res.ok) {
        ...
        // Notificar al componente padre para que también actualice
        if (onDataUpdated) {
          onDataUpdated();
        }
      } else {
