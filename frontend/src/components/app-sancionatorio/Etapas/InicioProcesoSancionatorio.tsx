...
export default function InicioProcesoSancionatorio({
  expedienteId,
  setToast,
  onStageUpdate,
  isEditable = true,
  ...
  const fetchInicioProceso = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDocumentos(res.inicio_proceso?.documentos_anexos || []);
  ...
  const creable = localCreable ?? inicioProcesoData?.creable ?? localInicioProceso?.creable ?? { status: true, msg: "" };
  ...
  const createStage = async () => {
    ...
    try {
      ...
      const res = await apiCall(
        API_CONFIG.ENDPOINTS.FILE_START_PROCESS_CREATE(expedienteId),
        { method: "POST" },
      );
            ...
            <div className="text-center space-y-3">
              ...
              <div className="bg-warning/10 border border-amber-200 rounded-lg p-4 max-w-lg mx-auto">
                <p className="text-sm font-medium">
                  <span className="font-semibold text-warning">Motivo:</span> {creable.msg}
                </p>
              </div>
  ...
  return (
    <div className="space-y-6">
      <ActoAdmin
        ...
        stageBinding={{ type: "etapa_san_inicio_sancionatorio", id: inicioProcesoData.etapa_id }}
        tipoEtapa="etapa_inicio_sancionatorio"
        setToast={setToast}
