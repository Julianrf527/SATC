...
export default function AperturaEtapaProbatoria({
  ...
  const [localApertura, setLocalApertura] = useState(inicio_proceso || null);
  ...
  const fetchApertura = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiCall(
        API_CONFIG.ENDPOINTS.FILE_OPENING_PROBATIONARY(expedienteId),
        {
          method: "GET",
        },
      );
      ...
      if (res.ok) {
        ...
        setDocumentos(res.apertura_etapa_probatoria?.documentos_anexos || []);

        if (res.apertura_etapa_probatoria?.acto_admin?.id) {
        ...
        setToast({
          id: Date.now(),
          message: res.detail || "Error al cargar apertura etapa probatoria.",
          type: "error",
        });
      }
    } catch (e) {
  ...
  const aperturaData =
    localApertura?.apertura_etapa_probatoria || localApertura;
  const stageExists = aperturaData && aperturaData.etapa_id;
  const creable = localCreable ?? aperturaData?.creable ?? localApertura?.creable ?? { status: true, msg: "" };
  ...
  const createStage = async () => {
    ...
    try {
      ...
      const res = await apiCall(
        API_CONFIG.ENDPOINTS.FILE_OPENING_PROBATIONARY_CREATE(expedienteId),
        { method: "POST" },
      );
      ...
      if (res.ok && res.etapa_id) {
        ...
        setLocalApertura(nuevaApertura);
        setDocumentos([]);
        setExistActoAdmin(false);
        ...
        <div className="card-body">
          <div className="flex flex-col items-center justify-center py-12 space-y-4">
            ...
            <p className="text-sm text-gray-500">
              Obteniendo información de la apertura etapa probatoria
            </p>
          </div>
        </div>
      </div>
    ...
    return (
      <div className="space-y-6">
        <ActoAdmin
          ...
          actoAdmin={aperturaData.acto_admin || {}}
          etapaId={aperturaData.etapa_id}
        stageBinding={{ type: "etapa_san_apertura_probatoria", id: aperturaData.etapa_id }}
