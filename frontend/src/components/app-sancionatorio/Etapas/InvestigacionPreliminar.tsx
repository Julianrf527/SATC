...
export default function InvestigacionPreliminar({
  ...
  const [localIndagacion, setLocalIndagacion] = useState(indagacion || null);
  ...
  const fetchInvestigation = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiCall(
        API_CONFIG.ENDPOINTS.FILE_INVESTIGATION(expedienteId),
        { method: "GET" },
      );

      if (res.ok) {
        setLocalIndagacion(res.indagacion);
        setDocumentos(res.indagacion?.documentos_anexos || []);

        if (res.indagacion?.acto_admin?.id) {
          setExistActoAdmin(true);
        } else {
          setExistActoAdmin(false);
        }
      }
      // ok: true, indagacion: null → etapa no existe aún, mostrar botón crear
    } catch (e) {
      setToast({ id: Date.now(), message: "Error al cargar la indagación preliminar.", type: "error" });
    } finally {
  ...
  const indagacionData = localIndagacion?.indagacion || localIndagacion;
  const stageExists = indagacionData && indagacionData.etapa_id;
  ...
  const createStage = async () => {
    ...
    try {
      ...
      const res = await apiCall(
        API_CONFIG.ENDPOINTS.FILE_INVESTIGATION_CREATE(expedienteId),
        { method: "POST" },
      );
        ...
        <div className="card-body">
          <div className="flex flex-col items-center justify-center py-12 space-y-4">
            ...
            <p className="text-sm text-gray-500">
              Obteniendo información de la indagación preliminar
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      <ActoAdmin
        ...
        actoAdmin={indagacionData.acto_admin || {}}
        etapaId={indagacionData.etapa_id}
        stageBinding={{ type: "etapa_san_indagacion", id: indagacionData.etapa_id }}
