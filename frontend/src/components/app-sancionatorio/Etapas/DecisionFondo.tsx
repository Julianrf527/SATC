...
import DataStageDecision from "./DecisionFondo/DecisionFondoData";
import DeleteStageModal from "./DecisionFondo/DeleteStageModal";
...
export default function DecisionFondo({
  ...
  const fetchDecision = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiCall(
        API_CONFIG.ENDPOINTS.FILE_DECISION(expedienteId),
        {
          method: "GET",
        },
      );
      ...
      if (res.ok) {
        ...
        setDocumentos(res.decision_fondo?.documentos_anexos || []);
  ...
  const creable = localCreable ?? decisionData?.creable ?? localDecision?.creable ?? { status: true, msg: "" };
  ...
  const hasNotificacionExitosa = () => {
    ...
    const involucrados =
      decisionData.acto_admin.notificacion.involucrados || [];
    return involucrados.some((inv: any) => inv.notificacion_exitosa === true);
  };

  // Combinar condiciones para mostrar acto de recurso
  const canCreateActoRecurso =
    creableActoRecurso.status && hasNotificacionExitosa();
  ...
  const createStage = async () => {
    ...
    try {
      ...
      const res = await apiCall(
        API_CONFIG.ENDPOINTS.FILE_DECISION_CREATE(expedienteId),
        { method: "POST" },
      );
  ...
  const handleActoAdminRecursoUpdate = async (
  ...
  if (stageExists) {
    return (
      <>
        ...
        stageBinding={{ type: "etapa_san_decision_fondo", id: decisionData.etapa_id }}
