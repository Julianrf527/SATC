...
export default function CierreEtapaProbatoria({
  ...
  const [localCierre, setLocalCierre] = useState(inicio_proceso || null);
  ...
  const fetchCierre = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiCall(
        API_CONFIG.ENDPOINTS.FILE_CLOSING_PROBATIONARY(expedienteId),
        {
          method: "GET",
        },
      );
      ...
      if (res.ok) {
        ...
        setDocumentos(res.cierre_etapa_probatoria?.documentos_anexos || []);

        if (res.cierre_etapa_probatoria?.acto_admin?.id) {
        ...
        setToast({
          id: Date.now(),
          message: res.detail || "Error al cargar cierre etapa probatoria.",
          type: "error",
        });
      }
    } catch (e) {
  ...
  const cierreData = localCierre?.cierre_etapa_probatoria || localCierre;
  const stageExists = cierreData && cierreData.etapa_id;
  const creable = localCreable ?? cierreData?.creable ?? localCierre?.creable ?? { status: true, msg: "" };
  ...
  const createStage = async () => {
    ...
    try {
      ...
      const res = await apiCall(
        API_CONFIG.ENDPOINTS.FILE_CLOSING_PROBATIONARY_CREATE(expedienteId),
        { method: "POST" },
      );
        ...
        <div className="card-body">
          <div className="flex flex-col items-center justify-center py-12 space-y-4">
            ...
            <p className="text-sm text-gray-500">
              Obteniendo información del cierre etapa probatoria
            </p>
          </div>
        </div>
      </div>
    ...
    return (
      <div className="space-y-6">
        <ActoAdmin
          ...
          etapaId={cierreData.etapa_id}
        stageBinding={{ type: "etapa_san_cierre_probatoria", id: cierreData.etapa_id }}
