...
export default function Document({
  ...
  const handleViewFile = (documentoAnexoId: number) => {
    openDocumentById(documentoAnexoId);
  };

  const handleSaveDocumento = async (
    payload: DocumentoSavePayload,
  ): Promise<{ ok: boolean; error?: string }> => {
    try {
      const isEditing = !!editingDocumento;
      const endpoint = isEditing
        ? API_CONFIG.ENDPOINTS.FILE_PUT_DOC_ATTACHED(
            etapaId,
            editingDocumento.id,
          )
      ...
      const documentoAnexoId =
        payload.documento_anexo_id ?? editingDocumento?.documento_anexo_id;
      if (!documentoAnexoId) {
      ...
      if (res.ok) {
        const documentoData = res.documento_anexo;
  ...
  const handleDeleteDocumento = async () => {
    try {
      ...
      const res = await apiCall(
        API_CONFIG.ENDPOINTS.FILE_DELETE_DOC_ATTACHED(
          etapaId,
          deletingDocumento.id,
        ),
        {
          method: "DELETE",
        },
            ...
            {sortedDocumentos.map((documento) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 ml-4">
                  ...
                  <button
                    onClick={() => handleViewFile(documento.documento_anexo_id)}
