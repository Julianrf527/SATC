...
import DataStageCessation from "./Cesacion/CesacionData";
...
export default function Cesacion({
  ...
  const [localCesacion, setLocalCesacion] = useState(inicio_proceso || null);
  ...
  const fetchCesacion = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiCall(
        API_CONFIG.ENDPOINTS.FILE_CESSATION(expedienteId),
        {
          method: "GET",
        },
      );
      ...
      if (res.ok) {
        ...
        setDocumentos(res.cesacion?.documentos_anexos || []);
        ...
        setToast({
          id: Date.now(),
          message: res.detail || "Error al cargar cesación.",
          type: "error",
        });
      }
    } catch (e) {
  ...
  const cesacionData = localCesacion?.cesacion || localCesacion;
  const stageExists = cesacionData && cesacionData.etapa_id;
  const creable = localCreable ?? cesacionData?.creable ?? localCesacion?.creable ?? { status: true, msg: "" };
  ...
  const createStage = async () => {
    ...
    try {
      ...
      const res = await apiCall(
        API_CONFIG.ENDPOINTS.FILE_CESSATION_CREATE(expedienteId),
        { method: "POST" },
      );
      ...
      if (res.ok && res.etapa_id) {
        ...
        setLocalCesacion(nuevaCesacion);
        setDocumentos([]);
        ...
        <div className="card-body">
          <div className="flex flex-col items-center justify-center py-12 space-y-4">
            ...
            <p className="text-sm text-gray-500">
              Obteniendo información de la cesación
            </p>
          </div>
        </div>
      </div>
  ...
  // Stage Cannot Be Created or Managed - Not Creable (prioritario, incluso si etapa_id existe)
  ...
  // Stage Doesn't Exist - Editable (Can be created) - solo si creable.status === true
    ...
    return (
      <div className="space-y-6">
        <ActoAdmin
          ...
          actoAdmin={cesacionData.acto_admin || {}}
          etapaId={cesacionData.etapa_id}
        stageBinding={{ type: "etapa_san_cesacion", id: cesacionData.etapa_id }}
