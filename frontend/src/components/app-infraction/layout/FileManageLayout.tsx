...
export default function FileManageLayout({ setToast }: Props) {
  ...
  }, [location.state?.radicadoToSelect, location.state?.timestamp, fileList]);
      ...
      try {
        if (user) {
          const res = await apiCall(
            API_CONFIG.ENDPOINTS.FILES_BY_USER(user.user_id)
          );
          if (res.ok) {
  ...
  const handleFileUpdate = (updatedFile: BasicFile) => {
    setFileList((prevList) =>
      prevList.map((f) =>
        f.radicado === selectedFile?.radicado ? { ...updatedFile } : f
      )
    );
  ...
  const handleArchiveSuccess = () => {
    ...
    setFileList((prevList) =>
      prevList.filter((f) => f.radicado !== selectedFile.radicado)
    );
