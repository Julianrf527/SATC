...
export default function Alerts({ radicado, setToast }: Props) {
  ...
  const loadAlertas = async () => {
    try {
      setIsLoading(true);
      const res = await apiCall(API_CONFIG.ENDPOINTS.FILE_ALERTS(radicado), {
        method: "GET",
      });
              ...
              alertasFiltradas.map((alerta, index) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-xs text-base-content/50">
                    ...
                    <span
                      className={`font-semibold ${alerta.semaforo.urgencia === "critico"
                          ? "text-error"
