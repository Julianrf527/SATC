...
export interface Props {
  ...
  selectedRadicados: Set<string>;
  onSelectionChange: (radicado: string, checked: boolean) => void;
...
export default function TableFiles({
  ...
  const allSelected = data.length > 0 && data.every((f) => selectedRadicados.has(f.radicado));
  const someSelected = data.some((f) => selectedRadicados.has(f.radicado));
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={titles.length + 1} className="text-center py-8">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span className="text-base-content/60">Cargando datos...</span>
                  </div>
                </td>
              </tr>
            ) : data.length === 0 ? (
              <tr>
                <td colSpan={titles.length + 1} className="text-center py-12">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span className="text-base-content/60 font-medium">No hay expedientes</span>
              ...
              data.map((f) => (
                <tr
                  key={f.radicado}
                  className={`hover cursor-pointer ${selectedRadicados.has(f.radicado) ? "bg-base-200" : ""}`}
                  onClick={() => onSelectionChange(f.radicado, !selectedRadicados.has(f.radicado))}
                >
                  <td onClick={(e) => e.stopPropagation()}>
                    <input
                      type="checkbox"
                      className="checkbox checkbox-sm"
                      checked={selectedRadicados.has(f.radicado)}
                      onChange={(e) => onSelectionChange(f.radicado, e.target.checked)}
                    />
                  </td>
                  <td className="select-text font-mono text-sm">{f.radicado}</td>
                  <td className="select-text">{f.nombre_expediente}</td>
                  <td className="select-text text-sm">{f.fecha_creacion.split("T")[0]}</td>
                  <td>
                    {f.encargado_nombre ? (
                      <span className="text-sm">{f.encargado_nombre}</span>
                    ) : (
                      <span className="text-base-content/40 text-sm italic">Sin asignar</span>
                    )}
                  </td>
                  <td>
                    {f.encargado_id ? (
        ...
        <div className="text-sm text-base-content/60">
          {selectedRadicados.size > 0 ? (
            <span className="text-success font-medium">
              {selectedRadicados.size} seleccionado{selectedRadicados.size !== 1 ? "s" : ""}
            </span>
          ) : (
