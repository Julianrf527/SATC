...
export default function OpeningProbationaryPeriod({
  radicado,
  idAuxiliar,
  setToast,
  onStageUpdate,
  ...
  const fetchApertura = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiCall(
        API_CONFIG.ENDPOINTS.FILE_OPENING_PROBATIONARY(radicado),
        {
          method: "GET",
        }
      );
  ...
  const tipoEtapaId =
    aperturaData?.tipo_etapa_id || localApertura?.tipo_etapa_id || null;
  const creable = aperturaData?.creable ||
    localApertura?.creable || { status: true, msg: "" };
    ...
    return (
      <div className="space-y-6">
        <ActoAdmin
          ...
          tipoEtapa="apertura_etapa_probatoria"
          setToast={setToast}
