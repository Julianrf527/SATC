...
import type { TipoNotificacion, Involved } from "../../../types/index";
import ActoAdmin from "./ActoAdmin/ActoAdmin";
import Documentos from "./Document/Document";
import DataStageCessation from "./Cessation/DataStageCessation";
...
export default function Cessation({
  ...
  const fetchCesacion = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiCall(API_CONFIG.ENDPOINTS.FILE_CESSATION(radicado), {
        method: "GET",
      });
  ...
  const tipoEtapaId =
    cesacionData?.tipo_etapa_id || localCesacion?.tipo_etapa_id || null;
  const creable = cesacionData?.creable ||
    localCesacion?.creable || { status: true, msg: "" };
