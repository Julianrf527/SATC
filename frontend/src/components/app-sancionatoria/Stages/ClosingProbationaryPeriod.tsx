...
export default function ClosingProbationaryPeriod({
  radicado,
  idAuxiliar,
  setToast,
  onStageUpdate,
  ...
  const fetchCierre = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiCall(
        API_CONFIG.ENDPOINTS.FILE_CLOSING_PROBATIONARY(radicado),
        {
          method: "GET",
        }
      );
  ...
  const tipoEtapaId =
    cierreData?.tipo_etapa_id || localCierre?.tipo_etapa_id || null;
  const creable = cierreData?.creable ||
    localCierre?.creable || { status: true, msg: "" };
    ...
    return (
      <div className="space-y-6">
        <ActoAdmin
          ...
          tipoEtapa="cierre_etapa_probatoria"
          setToast={setToast}
