...
import DataStageFormulation from "./Formulation/DataStageFormulation";
...
export default function FormulationCharges({
  radicado,
  idAuxiliar,
  setToast,
  onStageUpdate,
  ...
  const fetchFormulacion = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiCall(
        API_CONFIG.ENDPOINTS.FILE_FORMULATION(radicado),
        {
          method: "GET",
        }
      );
      ...
      if (res.ok) {
        ...
        if (res.formulacion?.acto_admin?.id) {
  ...
  const tipoEtapaId =
    formulacionData?.tipo_etapa_id || localFormulacion?.tipo_etapa_id || null;
  const creable = formulacionData?.creable ||
    localFormulacion?.creable || { status: true, msg: "" };
