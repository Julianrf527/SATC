...
import type { BasicFile, Town, Resource } from "../../../types";
...
type Props = {
  ...
  setSelectedFile: (file: BasicFile | null) => void;
  fileList: BasicFile[];
  setFileList: (files: BasicFile[]) => void;
  isEditable?: boolean;
};
...
export default function FileList({
  ...
  const [advancedFileList, setAdvancedFileList] = useState<BasicFile[]>([]);
  ...
  const availableTowns = useMemo(() => {
    ...
    const currentList = advancedFilters ? advancedFileList : fileList;
    currentList.forEach((f) => {
      if (f.municipio.name) {
        municipios.add(f.municipio.name);
      }
    });
    return Array.from(municipios);
  }, [fileList, advancedFileList, advancedFilters]);
  ...
  const applyAdvancedFilters = async (filters: FilterData) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setAdvancedFileList(response.data);
  ...
  const filteredFiles = useMemo(() => {
    ...
    return currentList.filter((f) => {
      ...
      const matchesTown =
        townFilter === "all" ? true : f.municipio.name === townFilter;

      const matchesStage =
        stageFilter === "all" ? true : f.ultima_etapa === stageFilter;
        ...
        >
          <div className="flex items-center justify-between text-sm">
            <span className="text-base-content/70">
              ...
              {advancedFilters ? advancedFileList.length : fileList.length}{" "}
              expedientes
            </span>
          ...
          {isLoadingAdvanced ? (
            ...
            filteredFiles.map((file) => (
              <FileCard
                key={file.radicado}
                title={file.radicado}
                expediente={file.nombre}
                fecha={file.fecha_creacion}
                municipio={file.municipio.name || "Desconocido"}
